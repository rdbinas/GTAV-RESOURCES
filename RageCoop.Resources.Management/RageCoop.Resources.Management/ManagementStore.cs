...
namespace RageCoop.Resources.Management{
	public class ManagementStore :IDisposable
	{
        ...
        {
			var query = new SQLiteCommand("IF NOT EXISTS " +
				$"(SELECT * FROM Banned WHERE IP = \"{ip}\") " +
				$"INSERT INTO Banned(IP, Username, Reason) VALUES(\"{ip}\", \"{username}\", \"{reason}\");",_con);
			query.ExecuteNonQuery();
        }
		public void Unban(string ip)
        {
			var query = new SQLiteCommand($"DELETE FROM Members WHERE IP = \"{ip}\";",_con);
			query.ExecuteNonQuery();
		}
